...
IUSE="aqua cleartype debug directfb doc gallium glitz udev opengl +svg X xcb"
...
RDEPEND="media-libs/fontconfig
	...
	gallium? ( >=media-libs/mesa-7.5[gallium] )
	glitz? ( >=media-libs/glitz-0.5.1 )
	udev? ( >=sys-fs/udev-136 )
...
src_configure() {
	...
	if use xcb && use udev; then
		xcb_drm="--enable-xcb_drm"
	fi

	econf $(use_enable X xlib) $(use_enable doc gtk-doc) \
		$(use_enable directfb) $(use_enable xcb) $(use_enable gallium) \
		$(use_enable svg) $(use_enable X xlib-xrender) $(use_enable glitz) \
		$(use_enable udev) ${xcb_drm} \
