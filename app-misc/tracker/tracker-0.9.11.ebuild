...
inherit eutils gnome2 linux-info autotools
...
IUSE="applet dbus doc eds exif flac gnome-keyring gsf gstreamer gif gtk gtk3 hal iptc +jpeg kmail laptop libicu +libunistring mp3 nautilus networkmanager pdf playlist rest rss strigi test +tiff +vorbis xine +xml xmp"

# Automagic, gconf, uuid, and probably more
# TODO: quill support
RDEPEND="
	>=app-i18n/enca-1.9
	>=dev-db/sqlite-3.6.17[threadsafe]
	...
	dbus? ( >=sys-apps/dbus-1.3.1 )
	eds? (
		!gtk3? (
			>=mail-client/evolution-2.25.5
			>=gnome-extra/evolution-data-server-2.25.5
			!>=mail-client/evolution-2.31
			!>=gnome-extra/evolution-data-server-2.31 )
		gtk3? (
			>=mail-client/evolution-2.31.3
			>=gnome-extra/evolution-data-server-2.31.4
		) )
...
pkg_setup() {
	...
	if use libunistring; then
		G2CONF="--with-unicode-support=libunistring"
	elif use libicu; then
		G2CONF="--with-unicode-support=libicu"
	else
		G2CONF="--with-unicode-support=glib"
	fi
	...
	G2CONF="${G2CONF}
		...
		$(use_enable dbus dbus-fd-passing)
...
src_prepare() {
	...
	epatch "${FILESDIR}/${P}-use-gtk3.patch"
